...
public class CategoriaController {
   ...
   // GET: obtener todas categorias
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> getById(@PathVariable("id") Integer categoria_id) {
        Categoria categoria = categoriaService.getById(categoria_id);
        if (categoria != null) {
            return ResponseEntity.ok(categoria);
    }     else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new z_Mensaje("Categoría no encontrada"));
    }
}
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> delete(@PathVariable("id") Integer categoria_id) {
    ...
    public Categoria obtenerHATEOAS(@PathVariable("id") Integer categoria_id) {
